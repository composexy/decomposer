...
internal abstract class AbstractKtorClient(
    ...
    private fun runSession(session: DefaultClientWebSocketSession) = with(session) {
        this@AbstractKtorClient.websocketSession = session
        launch(Job()) {
            while (true) {
    ...
    abstract override fun buildDeviceDescriptor(): DeviceDescriptor
